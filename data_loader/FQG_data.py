...
def prepro(config):
    ...
    if not config.processed_example_features:
        ...
        train_examples = prepro_examples(config, train_examples, emb_dicts )
        dev_examples = prepro_examples(config, dev_examples, emb_dicts )
        test_examples = prepro_examples(config, test_examples, emb_dicts)
...
def prepro_examples(config, examples, emb_dicts):
    num = len(examples)
    # num = 100
    ...
    if (config.use_refine_copy_tgt or
        ...
        # for i in range (300):
            ...
            # assign new values
            examples[i]["switch_oov"] = switch
            examples[i]["copy_position_oov"] = copy_position
